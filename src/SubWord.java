...
public class SubWord implements Comparable<SubWord> {
    ...
    public int compareTo(SubWord other)	{
        ...
        /*int a = 0;
        int b = 0;
        for(int i = 0; i < this.rootWord.length(); i++){
            a += alpha.indexOf(this.rootWord.substring(i, i +1));
        }
        for(int i = 0; i < other.rootWord.length(); i++){
            b += alpha.indexOf(other.rootWord.substring(i, i +1));
        }
        return a-b;*/
    }
